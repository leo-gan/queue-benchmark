...
name: prepare-pr
description: >
  End-to-end pre-PR gate for the queue-benchmark monorepo: refuse master/main,
...
cmake --build cpp/build -j"$(nproc 2>/dev/null || echo 2)" --target cpp_queue_tests
./cpp/build/cpp_queue_tests
```
...
- **regression (default):** fail only if error keys `(TestData, Queue, Mode)` appear that were **not** in the previous run for that language.
