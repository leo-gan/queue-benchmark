...
description: >
  Review queue-benchmark results for suspicious sizes/ops/fidelity, research
  each suspect against the library’s official docs and code examples, fix wrong or
  ineffective harness client call paths (without breaking queue isolation),
  ...
  benchmark numbers", "check outliers", "too fast / too slow queues", or
...
reading the **run harness** (cell construction / batch framing), not only queue
...
1. **Package docs + examples required before any client edit**  
   For each suspect queue, open and use **at least one** of:
...
2. **Isolation (C# and anywhere similar)**  
   Queue wrappers must **not** hard-code suite domain types (`Message`, `Telemetry`, …).  
...
6. **Harness runners are in scope, not only wrappers**  
   Suspects may live in `run_v2` / `batch_cell` / cell builders, not in `queues/*`.
...
- **Every** Rust queue shared the same under-encode path → relative ranking inside
  Rust looked “normal” (speedy still fastest among peers).
- Package-docs review of `rust/src/queues/speedy*` correctly concluded “API matches
...
3. Cross-lang: if one language’s median size for `fixture@n=N` is **≪ peer median / 5**, treat the **entire language harness** as suspect (runner/batch), not one queue.
4. Always open **runner + batch cell** code paths (table below), not only `queues/*`.
...
- Harness: switch (or add) row to `serde_avro_fast` with reused `QueueConfig` + `to_datum` / `from_datum_slice` ([docs](https://docs.rs/serde_avro_fast)).
...
| `BATCH-AXIS / LABEL≠WORK` | `n=N` size or ops not scaling vs `n=1` for same queue |
...
| **errors** | non-empty `logs/<lang>/<stem>.errors.csv` data rows for that queue |
| **fidelity oddity** | mean_fidelity &lt; 1 with large size, or fidelity 1.0 with tiny size |

#### B. Batch axis (same language, same queue, `n=1` vs `n=N`) — **required**
...
These fire even when **every** queue in the language is equally wrong.
...
Write a short **Suspect table**: language | queue **or harness** | signal | path to client **or runner** code.
...
|---------------|----------------|
| Within-group single queue | Package docs + that wrapper |
| BATCH-AXIS many queues same lang | **Runner / batch cell first**; docs second |
...
If harness is wrong, fix harness; do not churn queue wrappers or mark “expected” from crate docs alone.
...
| Lang | Queue wrappers | Runner / batch (LABEL≠WORK lives here) |
|------|---------------------|----------------------------------------|
| csharp | `c-sharp/src/Queues/*` | `c-sharp/src/*` run loop, config resolve, `DataTypeInstanceCount` usage |
| python | `python/src/benchmark/queues/*` | suite runner / instance list construction |
| rust | `rust/src/queues/*` | **`rust/src/run_v2.rs`**, `rust/src/data_v2.rs` (`make_one`) |
| go | `go/queues/*` | go runner / fixture batch build |
| javascript | `javascript/src/queues/*` | JS runner / fixture array for N |
| java | `java/src/main/java/benchmark/queues/*` | Main / Cells / instance count |
| kotlin | `kotlin/src/main/kotlin/benchmark/queues/*` | Main / Cells / instance count |
| cpp | `cpp/src/queues/*` | `register.cpp` / runner cells |
| c | `c/src/queues/*` | **`c/src/run_v2.c`**, **`c/src/batch_cell.c`** |
...
- Do not expand scope to unrelated queues.
...
```markdown
## Suspects found
| Lang | Queue / harness | Signal | Client path |

## Research & actions
### <queue or harness> (<lang>)
