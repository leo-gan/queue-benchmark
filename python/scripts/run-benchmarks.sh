...
case " $VALID_MODES custom " in
  *" $MODE "*) ;;
  *)
    echo "Usage: $0 [smoke|all-single|full|research|custom] [queueFilter] [dataFilter]"
    exit 1
    ;;
esac
...
if [[ "$MODE" == "custom" ]]; then
  ...
  if [[ "$MODE" == "smoke" ]]; then
    FILTER_SER="${FILTER_SER:-deque-lock}"
...
PYTHONPATH="$PROJECT_ROOT/analysis/src${PYTHONPATH:+:$PYTHONPATH}" \
  python3 "$PROJECT_ROOT/scripts/emit-cells-tsv.py" "$BENCHMARK_RUN_CONFIG" > "$CELLS_TSV" || true
...
if [[ -f "$CSV" ]]; then
  if PYTHONPATH="$PROJECT_ROOT/analysis/src${PYTHONPATH:+:$PYTHONPATH}" \
      python3 -m benchmark_analysis.environment "$CSV" >/dev/null 2>&1; then
    echo "[INFO] Run config captured -> ${CSV%.csv}.configs.json"
  else
    echo "[WARN] Could not write configs.json (analysis package optional)"
  fi
fi
